...
function read() {
  ...
  for (let index = 0; index < cartArray.length; index++) {
    ...
    $('#foodCa').append(
      ...
      `<p>碳水:${meal.MEAL_CARB}g</p>` +
      `<p>蛋白:${meal.MEAL_PROTEIN}g</p>` +
      `<p>脂肪:${meal.MEAL_FAT}g</p>` +
      `<p>${meal.MEAL_CARB * 4 + meal.MEAL_FAT * 9 + meal.MEAL_PROTEIN * 4}大卡</p>` +
      `</div>` +
      `<div class="col-2 align-items-center">` +
      `$` +
...
$(".discount").on("click", () => {
  ...
  if (getDiscount != "") {
    fetch(`http://localhost:8080/api/${userId}/coupons?couponname=${getDiscount}&token=${token}`,
      {
        method: "GET"
...
function checkOutFinal(userId, token) {
  ...
  if (shoppingCar.length > 0) {
    ...
    if ($("[name='flexRadioDefault']:checked").val() === "2") {
      ...
      fetch(`http://localhost:8080/api/${userId}/stripepayment?token=${token}`, {
        method: "post",
